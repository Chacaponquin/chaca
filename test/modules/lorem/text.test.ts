...
describe("# Lorem text test", () => {
  ...
  it("Without arguments", () => {
    const allTexts = Array.from({ length: ARRAY_LENGTH_TEXT }).map(() => {
      return modules.lorem.text().getValue();
    });
  ...
  it("With character_min=5", () => {
    const allTexts = Array.from({ length: ARRAY_LENGTH_TEXT }).map(() => {
      return modules.lorem.text({ character_min: 5 }).getValue();
    });
  ...
  it("With character_max=10", () => {
    const allTexts = Array.from({ length: ARRAY_LENGTH_TEXT }).map(() => {
      return modules.lorem.text({ character_max: 10 }).getValue();
    });
